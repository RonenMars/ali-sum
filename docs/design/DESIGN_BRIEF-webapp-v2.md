# ali-sum — Web App Design Brief

**Product:** ali-sum — AliExpress Order History Analytics
**Surface:** Web dashboard (desktop-first, mobile supported)
**Date:** April 2026
...
**This is a greenfield design.** No existing UI, no established brand, no committed color palette. You are defining the visual identity from scratch, constrained only by the product principles and semantic requirements described in this brief.
...
**Semantic color vocabulary (to be designed, not pre-defined):** The status system requires a consistent color vocabulary across every surface — cards, charts, badges, filter pills. The specific hues are yours to define, but the semantic mapping is fixed:

| Meaning | Requirement |
|---------|-------------|
| Delivered / Success | One distinct color — used consistently everywhere for this meaning |
| In Transit | One distinct color — used consistently everywhere for this meaning |
| Awaiting Delivery | A variant of In Transit — lighter or muted |
| Ready to Ship | One distinct color — used consistently everywhere for this meaning |
| Pending / Processing | One distinct color — used consistently everywhere for this meaning |
| Error | One distinct color — used consistently everywhere for this meaning |
| Running / Syncing | Animated treatment of the In Transit color |

The brand accent color (your choice) must not be used for data encoding — that would create confusion between brand identity and semantic meaning.

**Open questions — what's yours to resolve vs. what needs a stakeholder decision first:**

| Question | Owner |
|----------|-------|
| Navigation order (should Shipping move up in the nav?) | **Stakeholder** — don't commit to a layout until resolved |
...
ali-sum is a personal analytics dashboard for AliExpress shoppers. A companion Chrome extension (designed separately — see the Extension Design Brief) scrapes a user's order history from AliExpress and syncs it to this dashboard. From that point on, the dashboard gives the user a living view of their spending habits, package statuses, and seller relationships — all in one place.

The core value proposition is **clarity**: turning a noisy, hard-to-search order history into answers.

The web dashboard is the primary experience. The extension is infrastructure — it feeds data in. Users spend their time here.
...
Six sections and their actual priority to users. The conceptual nav order (Overview → Spending → Sellers → Orders → Shipping → Settings) does not match usage behavior — Shipping is likely the most-visited page for active users yet sits near the bottom. Whether to reorder is a stakeholder decision (see Executive Summary).
...
- Empty state (no synced orders yet) must clearly guide the user toward setting up the extension — this is the most important empty state in the product.
...
**How it should feel:**
- Seller names can be long. The chart must handle truncation gracefully, with tooltips for full names.
...
- Status badges use the same semantic color vocabulary as everywhere else in the app.
...
- A package table grouped by tracking number, with collapsible rows that expand to show individual orders under that shipment
...
- Status colors must be consistent everywhere — the same color for "In Transit" appears on the card, the chart, the badge, and the filter pill.
...
- Sync history helps users debug ("did my sync work?") — success and failure states need to be immediately readable. Color-coded: success is reassuring, error is clear, running is animated.
- This page doesn't need to be visually interesting. Calm and utilitarian is exactly right.

---

## 6. Component Intentions
...
All charts share a consistent visual language: the same semantic color vocabulary, formatted tooltips, graceful empty states, and skeleton loaders that match each chart's shape.
...
| Running / Syncing | In progress (animated) |

### Sidebar Navigation

Fixed on desktop. Slide-in drawer on mobile with backdrop overlay. The active item has a filled background — not just a dot or underline. Icon and label are always visible together. User identity pinned to the bottom. Brand mark at the top.
...
No color palette is pre-defined — this is yours to establish. The constraints:

- **Brand accent:** One primary color for interactions, active states, and CTAs. Must not be reused for semantic data encoding in charts or badges.
- **Semantic palette:** Define one color per meaning from the table in the Executive Summary. These must be internally consistent — the same hue appears everywhere a given status is communicated.
- **Surface hierarchy:** Three distinct levels — background, card, elevated card. Readable without relying on borders alone.
- **Text hierarchy:** Three distinct weights — primary, secondary, muted.
- **Dark mode:** Design it as a first-class variant, not an afterthought inversion of light mode.

### Typography

Numbers — especially currency amounts — need to feel authoritative. Use tabular figures so values align vertically in tables and don't shift width between amounts. Tracking numbers and order IDs should visually read as technical strings, distinct from surrounding prose — a monospaced or distinct treatment works well here. At least three text weights are needed for meaningful hierarchy across labels, values, and supporting text.
...
- **New user, no synced data** — the most important empty state in the product. Must not feel like a broken page. Should read as an invitation: "Your dashboard is ready. Here's how to fill it." Guide the user toward the extension setup with a clear CTA.
...
A brand new user who registers and lands on the dashboard has no data. This is the most critical first impression — it must feel like an invitation, not a broken page.

**The intended flow:**

1. **Register** → land on the dashboard
2. **Dashboard (empty state)** — "Your dashboard is empty. Connect the Chrome extension to import your AliExpress orders." → CTA leads to Settings
3. **Settings** walks the user through the connection in explicit steps:
   1. Install the ali-sum Chrome extension from the Chrome Web Store
   2. Generate an API token in Settings
   3. Copy the token
   4. Open the extension popup and paste the token
   5. Navigate to AliExpress order history and click "Sync Now" in the extension popup
4. **Post-sync** — the dashboard populates automatically, with a clear signal that data has arrived

Consider a persistent "Get started" checklist or progress banner that stays visible until the first sync is complete, so the user always knows where they are in the setup process.
...
*This brief is a starting point, not a cage. Challenge any assumption — especially around information architecture and layout — if there's a better experience to be found. Resolve the stakeholder-owned open questions before committing to layouts that depend on them.*
