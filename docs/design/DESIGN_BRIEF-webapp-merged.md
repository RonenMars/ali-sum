...
ali-sum is a personal analytics dashboard for AliExpress shoppers. A companion browser extension pulls in a user's order history automatically. From that point on, the dashboard gives them a living view of their spending habits, package statuses, and seller relationships — all in one place.
...
The core value proposition is **clarity**: turning a noisy, hard-to-search order history into answers. How much am I spending? Where are my packages? Who do I buy from most?

---

## 2. The Users

**Primary: The Frequent Shopper**

Orders regularly from AliExpress — frustrated by the platform's own UI, which makes it nearly impossible to search history, understand spending patterns, or track multiple packages at once. Wants a clean personal-finance-adjacent view of their activity. Comfortable with technology but not a developer.

Their mental model is something like Mint or Copilot for bank transactions: take the raw data and turn it into patterns without them having to think about it.

**Key questions they're trying to answer:**
...
Buys in bulk from a consistent set of sellers. Cares about seller-level spend breakdowns and fast access to tracking numbers. More analytical than the average shopper.
...
The current palette uses a warm orange primary. Dark mode is structurally supported. The typography is clean but undifferentiated — no hierarchy beyond size.
...
1. **Trust at a glance.** The moment a user lands, they should feel: "my data is here, organized, and correct." Numbers need to feel authoritative — precise, clearly labeled, no ambiguity about what a figure represents.

2. **Spending clarity above all.** The most valuable thing this product can do is answer "how much have I spent this month" in under three seconds. The design should make that effortless.

3. **Package anxiety relief.** For active shoppers, "where is my stuff" is a daily question. The shipping section should feel like a calm status board — organized, reassuring, and immediately readable.

4. **Progressive disclosure.** Summaries first, details on demand. Users shouldn't be overwhelmed on first view — they drill in when they care.

### Secondary

- Feel **personal, not enterprise.** This is a tool for one person's data. Warm and approachable, not B2B SaaS.
- **Dark mode should be graceful**, not an afterthought. Many users will use this at night.
- **Empty and loading states matter.** A new user with no synced data is a critical first impression — it should feel like an invitation, not a broken page.
- **Mobile-readable**, even if not mobile-first. The experience degrades gracefully on smaller screens.

---

## 5. Information Architecture

The six sections and their actual priority to users:
...
The current nav order (Overview → Spending → Sellers → Orders → Shipping → Settings) does not match this priority. Shipping is likely the most-visited page for active users, yet it sits near the bottom. This is an open question worth challenging.
...
- Consider whether a cumulative view (total to date) alongside a per-period view would add value.
...
- A full sortable list with order count and total spent per seller

**How it should feel:**
- Seller names can be long. The chart needs room for them.
- The visual and the table show the same data. They should feel complementary — the chart for visual comparison, the table for exact numbers. If one isn't earning its space, remove it.
- Linking seller names to their storefronts would be high value.
...
- A search bar for seller name or order ID lookup would significantly reduce the burden of browsing a long table.
...
**How it should feel:**
- This is the most complex page in the product. Interactions must be crystal clear.
...
- Status colors must be consistent everywhere in the app:
  - In Transit → Blue
  - Delivered → Green
...
Preset options: This week, Last 7 days, This month, Last 30 days, Last 3 months, This year. Plus a custom from/to input. The active preset should look clearly selected — filled, not just underlined. The default is "This month." The filter persists across page navigations so the user's context doesn't reset. On mobile it should collapse into a compact control.
...
Fixed on desktop. Slide-in drawer on mobile. The active item has a filled background — not just a dot or underline. Icon and label are always visible together. User identity pinned to the bottom. Brand mark at the top, anchoring the layout without competing with content.
...
This should feel like a well-organized personal finance tool — not a toy, not an enterprise dashboard. Reference points: Monarch Money, Copilot, Linear. Clean, opinionated design that takes data density seriously.
...
The warm orange primary is already established and worth keeping as the brand accent. The full palette should be built around it with intention:
...
- **Semantic colors:** Green for delivered/success, Blue for in transit/info, Amber for pending/warning, Red for error — consistent everywhere, every time.
...
Numbers — especially currency amounts — need to feel authoritative. This means numerals that align vertically in tables and don't shift width between values. Tracking numbers and order IDs should visually read as technical strings, distinct from surrounding text. At least three text weights are needed for meaningful hierarchy.

### Spacing and Layout

Cards should breathe — generous internal padding. Charts should never feel cramped against card edges. Table rows need enough height for comfortable scanning without feeling padded for its own sake. The overall layout should reward calm reading.

### Motion

Motion should be minimal and purposeful. Page transitions: none, or nearly none — this is a data tool, not a marketing site. Chart entrance animations: subtle, on initial load only, not on every filter change. Loading states use skeleton loaders shaped like the content they replace. Collapsible rows animate smoothly. Copy-to-clipboard gets a brief visual confirmation, then rests. Every animation should feel like the UI breathing, not performing.

---

## 9. Responsive Behavior

**Mobile:** Sidebar hidden behind a hamburger. Single-column stat cards. Charts simplified or horizontally scrollable. Tables show only the most essential columns — others collapse or move to a detail row. The date filter collapses into a single control.
...
**Wide desktop:** Content is centered with a maximum width — it should not stretch indefinitely.

**A broader question:** Should mobile get a purpose-built layout — optimized around "where are my packages?" — rather than a responsive adaptation of the desktop experience? Mobile use is more focused, and the desktop IA may not serve it well.

---

## 10. Chrome Extension Popup

The popup is a compact, single-purpose surface. No navigation. Its job is to trigger syncs, show progress, and confirm results.
...
The popup is used while the user is actively on AliExpress — they want to kick off a sync and know it's working. It should be fast to glance at. Same visual language and color meaning as the web app.
...
Consider a persistent "Get started" banner or progress checklist that stays visible until the first sync is complete — so the user always knows where they are in the setup.
...
## 14. Open Questions for the Designer

These are intentionally unresolved. The designer should feel empowered to challenge any assumption here:

1. **Navigation order:** Should the nav reflect actual user behavior — Shipping near the top — rather than the current conceptual hierarchy?

2. **Dashboard density:** Should the Overview page be high-density (everything at a glance) or more editorial (one focal insight at a time, scrollable)?

3. **Sellers page redundancy:** The chart and the table show the same data. Does the chart earn its space, or should one be removed in favor of the other?

4. **Global date context:** The filter is currently per-page. Would a global date context — always visible, always in sync across sections — better serve a "reporting" mental model?

5. **Personalized insights:** Could the dashboard surface contextual insights ("You've spent 40% more this month than last month") as a distinct pattern, separate from raw numbers?

6. **Extension popup depth:** After a sync, should the popup show a mini-dashboard preview (recent orders, in-transit count) or stay purely functional?

7. **Mobile as a distinct surface:** Should mobile get a purpose-built layout optimized for "where are my packages?" rather than a responsive adaptation of the desktop dashboard?

---

*This brief is a starting point, not a cage. Challenge any assumption — especially around information architecture and layout — if there's a better experience to be found.*
